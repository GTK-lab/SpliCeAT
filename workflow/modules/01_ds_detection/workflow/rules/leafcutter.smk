...
rule leafcutter_exons:
	...
	output:
		os.path.join(LC_DIR,"leafcutter_exons.txt.gz")
	conda:
...
rule regtools_junction_extract:
	input:
		bam=get_bam
	output:
		os.path.join(RT_DIR,"{sample}.junc")
	container:
...
rule create_leafcutter_junction_file:
	input:
		expand(os.path.join(RT_DIR,"{sample}.junc"),sample=SAMPLES),
	output:
		temp(os.path.join(LC_DIR,"junction_files.txt"))
	log:
...
rule leafcutter_cluster:
	...
	output:
		counts=os.path.join(LC_DIR,"leafcutter_perind_numers.counts.gz"),
		pooled=os.path.join(LC_DIR,"leafcutter_pooled"),
	#shadow: "full",
	params:
...
rule leafcutter_differential_splicing:
	input:
		counts=os.path.join(LC_DIR,"leafcutter_perind_numers.counts.gz"),
		pooled=os.path.join(LC_DIR,"leafcutter_pooled"),
		exons=os.path.join(LC_DIR,"leafcutter_exons.txt.gz"),
	output:
		signif=os.path.join(LC_DIR,"leafcutter_cluster_significance.txt"),
		effect_size=os.path.join(LC_DIR,"leafcutter_effect_sizes.txt")
	params:
		min_samples_per_group=config["leafcutter"]["min_samples_per_group"],
		groups_file = leafcutter_grouppath,
		prefix = os.path.join(LC_DIR, "ds")
	conda:
		"../../../../envs/leafcutter.yaml"
	log:
...
rule leafcutter_lsvs:
	input:
		...
		effect_size=os.path.join(LC_DIR,"leafcutter_effect_sizes.txt"),
	output:
		lsvs=os.path.join(LC_DIR,"leafcutter_lsvs.tsv"),
	log:
